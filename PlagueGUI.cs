...
namespace SurviveThePleasure
{
    public static class PlagueGUI
    {
        ...
        public static DropDownData DropDown(Rect PositionAndScale, string MainButtonText, List<KeyValuePair<string, Action>> Buttons)
        {
            ...
            if (DropDownState.ContainsKey(DropData))
            {
                if (DropDownState[DropData].IsOpen)
                {
                    DropDownState[DropData].ScrollPosition = GUI.BeginScrollView(new Rect(PositionAndScale.x, PositionAndScale.y + 25, PositionAndScale.width + 25, 700), DropDownState[DropData].ScrollPosition, new Rect(0, 0, PositionAndScale.width + 25, (PositionAndScale.height * (Buttons.Count + 1)) + 25));
