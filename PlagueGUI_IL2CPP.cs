...
namespace PlagueGUIIL2CPP
{
    public static class PlagueGUIIL2CPP
    {
        /// <summary>
        ...
        public static DropDownData DropDown(Rect PositionAndScale, string MainButtonText, List<KeyValuePair<Tuple<string, string, ButtonType, bool>, Action<string, int, float, bool>>> Buttons, bool ShowSearch = true, bool Sort = true)
        {
            ...
            if (DropDownState[DropData].IsOpen && ShowSearch)
            {
                ...
                GUI.Label(new Rect(PositionAndScale.x, PositionAndScale.y + 25, PositionAndScale.width, PositionAndScale.height), new GUIContent("", null, "Search The DropDown List For An Item"));
            }

            if (GUI.Button(PositionAndScale, new GUIContent((DropDownState[DropData].IsOpen ? @"/\ " : @"\/ ") + MainButtonText + (DropDownState[DropData].IsOpen ? @" /\" : @" \/"), null, (DropDownState[DropData].IsOpen ? "Closes The DropDown" : "Expands The DropDown"))))
            {
                    ...
                    void MakeButton()
                    {
                        switch (Button.Key.Item3)
                        {
                            case ButtonType.Button:
                                if (GUI.Button(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), new GUIContent(Button.Key.Item1, null, Button.Key.Item2)))
                                {
                            ...
                            case ButtonType.Toggle:
                                if (GUI.Toggle(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), DropDownState[DropData].BoolCache[ControlID], new GUIContent(Button.Key.Item1, null, Button.Key.Item2)) != DropDownState[DropData].BoolCache[ControlID])
                                {
                            ...
                            case ButtonType.Radial:
                                if (GUI.Toggle(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), DropDownState[DropData].RadialBoolCache[ControlID], new GUIContent(Button.Key.Item1, null, Button.Key.Item2)) != DropDownState[DropData].RadialBoolCache[ControlID])
                                {
                            ...
                            case ButtonType.Slider:
                                GUI.Label(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), new GUIContent(Button.Key.Item1, null, Button.Key.Item2));
                                ...
                                GUI.Label(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), new GUIContent("", null, "Current Value: " + ((int)DropDownState[DropData].FloatCache[ControlID]).ToString()));
                            ...
                            case ButtonType.TextArea:
                                ...
                                GUI.Label(new Rect(0, 25 * ButtonPosMultiplier, PositionAndScale.width, PositionAndScale.height), new GUIContent("", null, Button.Key.Item2));
